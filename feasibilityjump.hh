...
struct Problem
{
	std::vector< FJ_Var > vars;
	std::vector< FJ_Constraint > constraints;
	...
	int addConstraint(RowType sense,
		double rhs,
		int numCoeffs,
		int* rowVarIdxs,
		double* rowCoeffs,
	...
	void resetIncumbent(double* initialValues)
	{
		...
		{
			FJ_Constraint& cstr = constraints[cIdx];
...
{
	FJ_Constraint& c = problem.constraints[mod.constraintIdx];
			...
			for (auto& cIdx : problem.violatedConstraints)
			{
				...
				for (auto& cell : constraint.coeffs)
				{
					forEachMove(
						cell.idx, [&](Move& move)
						{
						  double candidateLhs = constraint.incumbentLhs
							  + cell.coeff * (move.value - problem.incumbentAssignment[cell.idx]);
